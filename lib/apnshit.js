...
(function() {
  ...
  Apnshit = (function(_super) {
      ...
      return this.connect_promise || (this.connect_promise = defer(function(resolve, reject) {
        ...
        if (_this.socket && _this.socket.writable) {
          ...
          _this.socket = tls.connect(_this.options.port, _this.options.gateway, socket_options, function() {
            ...
            resolve();
            return _this.emit("connect#connected");
          });
    ...
    Apnshit.prototype.events = ['connect#start', 'connect#exists', 'connect#connecting', 'connect#connected', 'disconnect#start', 'disconnect#drop', 'disconnect#drop#infinite_resend', 'disconnect#drop#infinite_resend#limit_reached', 'disconnect#drop#resend', 'disconnect#drop#nothing_to_resend', 'disconnect#finish', 'reset#start', 'reset#socket', 'reset#socket#close', 'send#start', 'send#connected', 'send#write', 'send#write#finish', 'socket#error', 'socketData#start', 'socketData#found_intentional_bad_notification', 'socketData#found_notification', 'watchForStaleSocket#start', 'watchForStaleSocket#interval#start', 'watchForStaleSocket#interval#socket_not_writable', 'watchForStaleSocket#interval#stale', 'watchForStaleSocket#interval#stale#no_response', 'watchForStaleSocket#interval#stale#intentional_bad_notification'];
      ...
      return this.reset({
        ...
        if (options.drop) {
          ...
          if (resend) {
            ...
            resend = _.reject(resend, function(n) {
              return n.alert === 'x';
            });
    ...
    Apnshit.prototype.reset = function(options) {
      ...
      return defer(function(resolve, reject) {
        var close, close_timer;
        _this.emit("reset#start");
        ...
        if (options.socket) {
          _this.emit("reset#socket");
          delete _this.connect_promise;
          if (_this.socket) {
            close = function() {
              _this.emit("reset#socket#close");
    ...
    Apnshit.prototype.send = function(notification) {
      ...
      this.not_sure_if_sent.push(notification);
    ...
    Apnshit.prototype.watchForStaleSocket = function() {
      ...
      return this.interval = setInterval(function() {
        var noti, stale;
        _this.emit('watchForStaleSocket#interval#start');
        if (_this.socket && !_this.socket.writable) {
          _this.emit('watchForStaleSocket#interval#socket_not_writable');
          _this.disconnect({
            drop: true
          });
          return;
        ...
        _this.emit('watchForStaleSocket#interval#stale', stale);
        if (stale) {
          _this.stale_count || (_this.stale_count = 0);
          _this.stale_count++;
          if (_this.bad_alert_sent) {
            _this.emit('watchForStaleSocket#interval#stale#no_response');
            _this.disconnect({
              drop: true
            });
          } else if (_this.stale_count === 2) {
            _this.emit('watchForStaleSocket#interval#stale#intentional_bad_notification');
