    ...
    Apnshit.prototype.disconnect = function(options) {
      ...
      if (options.drop) {
        ...
        if (resend) {
          ...
          this.last_resend_uids || (this.last_resend_uids = []);
          resend_uids = _.map(resend, function(n) {
            return n._last_uid;
          });
          if (!(resend_uids < this.last_resend_uids || this.last_resend_uids < resend_uids)) {
            this.emit("disconnect#drop#infinite_resend", resend);
            this.infinite_resend_count || (this.infinite_resend_count = 0);
            this.infinite_resend_count++;
            if (this.infinite_resend_count === this.options.infinite_resend_limit) {
              this.emit("disconnect#drop#infinite_resend#limit_reached", resend);
              this.emit("dropped", resend);
              delete this.last_resend_uids;
              return;
            }
          } else {
            this.infinite_resend_count = 0;
          }
          this.emit("disconnect#drop#resend", resend);
          this.last_resend_uids = _.map(resend, function(n) {
            return n._uid;
          });
          return setTimeout(function() {
            ...
            return _results;
          }, 500 * this.infinite_resend_count);
        } else {
